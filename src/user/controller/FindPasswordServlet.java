...
//		
		...
		//4. 응답처리
//		if(u != null
//			&& userId.equals(u.getUserId())
//			&& userEmail.equals(u.getEmail())) {
//			
//			//임시 비밀번호 발급 업무로직 적을 곳!
//			System.out.println("임시 비밀번호를 발급하였습니다.");
...
//		else {
//			//일치하지 않을 시 돌아감!
//			request.setAttribute("msg", "아이디 또는 이메일 주소가 일치하지 않습니다.");
...
//            request.getRequestDispatcher("/views/login_myPage/searchPasswordEnd.jsp").forward(request, response);
            ...
            String newPwd = Utils.getEncryptedPassword(AuthenticationKey);
            int result = new UserService().updatePassword(u.getUserId(), newPwd);
            String chk = result>0? "비밀번호 변경 성공!" : "비밀번호 변경 실패!";
